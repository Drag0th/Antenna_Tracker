...
void display_data(telemetry_data *data, SSD1306AsciiWire &oled_display){
  oled_display.clear();
  printL(data->lat_array[data->loop_counter], oled_display); //gps
  oled_display.print(" ");
  printL(data->lon_array[data->loop_counter], oled_display);
...
void display_average_data(telemetry_data *data, SSD1306AsciiWire &oled_display){
  oled_display.clear();
  printL(data->average_lat, oled_display); //gps
  oled_display.print(" ");
  printL(data->average_lon, oled_display);
  oled_display.println();
...
void display_calibration_message(SSD1306AsciiWire &oled_display) {
  ...
  oled_display.set2X();
  oled_display.clear();
  oled_display.println("INITIAL");
  oled_display.println("CALIBRATION");
  oled_display.set1X();
  oled_display.setFont(System5x7);
}
...
void display_telemetry_lost(SSD1306AsciiWire &oled_display) {
  ...
  oled_display.set2X();
  oled_display.clear();
  oled_display.println(" TELEMETRY");
  oled_display.println("    LOST   ");
  oled_display.set1X();
  oled_display.setFont(System5x7);
